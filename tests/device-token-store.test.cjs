...
runTest('Test 33: upsert rejects pathologically oversized fields (DoS guard)', (s) => {
  assertThrows(
    () => s.upsert(makeInput({ token: 'x'.repeat(513) })),
    'token > 512 chars throws'
  );
  assertThrows(
    () => s.upsert(makeInput({ deviceId: 'd'.repeat(257) })),
    'deviceId > 256 chars throws'
  );
  assertThrows(
    () => s.upsert(makeInput({ friendlyName: 'n'.repeat(257) })),
    'friendlyName > 256 chars throws'
  );
  assertThrows(
    () => s.upsert(makeInput({ bundleId: 'b'.repeat(257) })),
    'bundleId > 256 chars throws'
  );
  // At-limit values are accepted (boundary).
  const rec = s.upsert(makeInput({ token: 't'.repeat(512), deviceId: 'd'.repeat(256) }));
  assertNotNull(rec.id, 'at-limit token+deviceId accepted');
});
