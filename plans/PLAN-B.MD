> **Note**: Some architectural choices have evolved. The system now uses adapter-node + custom server.ts instead of Vercel. Terminal access and WebSocket features described in later phases are now implemented.
...
export class APNsClient {
  ...
  async sendNotification(
    ...
    const host = this.isDevelopment ? 'api.sandbox.push.apple.com' : 'api.push.apple.com';
...
describe('Happy Path Integration', () => {
  it('should deliver notification end-to-end', async () => {
    ...
    const actionResponse = await simulateIOSAction(notificationResponse.notificationId, 'confirm');
    expect(actionResponse.delivered).toBe(true);

    // 4. Verify webhook delivery (Phase 3+)
    const webhookReceived = await waitForWebhook(notificationResponse.notificationId);
...
export default function () {
  ...
  const res = http.post('https://your-app.vercel.app/api/notify', payload, params);
...
function checkEnvironment(): boolean {
  const required = ['API_KEY', 'DEVICE_TOKEN', 'APNS_KEY_ID', 'APNS_TEAM_ID', 'APNS_KEY'];
